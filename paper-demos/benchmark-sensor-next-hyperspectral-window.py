...
def train_model(arch, dataset, sensor_kind):

    # build dataset with specific sensor type
    sensor_next_window_dataset = SensorPlacementDataset(
            dataset, n_sensors=50, radius=5.5, 
            kind=kind, failure_rate=0.2, return_mask=False, make_cache=False)
    dls = create_dataloaders_from_dataset(sensor_next_window_dataset, batch_size=args.batch_size)
    ...
    learner = Learner(model=model, dls=dls, **learner_kwargs)
    ...
    callbacks = [
        ...
        CSVLogger(fname=str(Path(learner_kwargs['path']) / learner_kwargs['model_dir'] /'train_history.csv'))
    ]
...
print(f'Starting experiments on next window dataset. ',
      f'Training for {args.n_epochs} epochs with \n{experiments}')

for arch, kind in experiments:
    name = 'unet_' + arch
    print('\n'*5, f'Starting {name} with {kind} placement and batch size {args.batch_size}.'.center(60, '-'))

    learner_kwargs = dict(path=Path(code_root)/ 'models' / 'sensor_next_window' / f'{kind}_sensors',
                        model_dir=name,
                        loss_func=nn.MSELoss()
                        )

    if not (learner_kwargs['path']/learner_kwargs['model_dir']).exists():
        (learner_kwargs['path']/learner_kwargs['model_dir']).mkdir(parents=True, exist_ok=True)
    train_model(arch, next_window_train, kind)

    print(f'Finished {name}with {kind} placement.')